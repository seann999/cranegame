...
public class MoveClaw : MonoBehaviour {
	...
	public void Move(float x, float y, float z, int moveMode, bool open) {
		...
		if (moveMode == 2) {
			...
			//move = Vector3.MoveTowards(body.position, target, 0.01f);
		...
		/*if (command == "forward") {
			...
			bodyRB.constraints = RigidbodyConstraints.FreezePositionY | RigidbodyConstraints.FreezePositionX | ;
		} else if (command == "left") {
			move = new Vector3 (-1f, 0, 0);
			bodyRB.constraints = RigidbodyConstraints.FreezePositionY | RigidbodyConstraints.FreezePositionZ | RigidbodyConstraints.FreezeRotation;
		} else if (command == "backward") {
			move = new Vector3 (0, 0, -1f);
			bodyRB.constraints = RigidbodyConstraints.FreezePositionY | RigidbodyConstraints.FreezePositionX | RigidbodyConstraints.FreezeRotation;
		} else if (command == "right") {
			move = new Vector3 (1f, 0, 0);
			bodyRB.constraints = RigidbodyConstraints.FreezePositionY | RigidbodyConstraints.FreezePositionZ | RigidbodyConstraints.FreezeRotation;
		} else if (command == "up") {
			move = new Vector3 (0, 1f, 0);
			bodyRB.constraints = RigidbodyConstraints.FreezePositionZ | RigidbodyConstraints.FreezePositionX | RigidbodyConstraints.FreezeRotation;
		} else if (command == "down") {
			move = new Vector3 (0, -1f, 0);
			bodyRB.constraints = RigidbodyConstraints.FreezePositionZ | RigidbodyConstraints.FreezePositionX | RigidbodyConstraints.FreezeRotation;
		} else if (command == "target") {
			move = (target - bodyRB.position);
			move.y = 0;
			float a = 5;
			...
			bodyRB.constraints = RigidbodyConstraints.FreezePositionY | RigidbodyConstraints.FreezeRotation;
		}*/
