...
class RNNLearner():
    def __init__(self):
        ...
        flags.DEFINE_boolean("pretrain", True, "pretrain")
        ...
        if not pretrain:
            for i in range(FLAGS.threads):
                game_processes.append(subprocess.Popen("./game.x86_64 %s 50 10 1 aaaaa" % str(5000 + i),
        ...
        with tf.device('/cpu:0'):
            ...
            for i in range(FLAGS.threads):
                worker = a3c.Worker(i, 2, trainer, global_episodes, not pretrain)
        ...
        if pretrain:
            try:
                master_network.pretrain(global_episodes, self.sess, writer)
            except KeyboardInterrupt:
                print("saving")
                saver.save(self.sess, "%s/model" % FLAGS.model_dir, self.sess.run(global_episodes))
                print("saved")
